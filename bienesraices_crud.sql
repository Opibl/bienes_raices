...
CREATE TABLE propiedades (
  ...
  precio DECIMAL(10, 2),
  imagen VARCHAR(200),
  descripcion TEXT, -- 'longtext' se convierte en 'TEXT' en PostgreSQL
  habitaciones INT,
  wc INT,
  estacionamiento INT,
  vendedorId INT,
  creado DATE,
  CONSTRAINT fk_vendedor FOREIGN KEY (vendedorId) REFERENCES vendedores(id) -- Crear la restricción de clave foránea
);

-- Crear la tabla 'usuarios'
CREATE TABLE usuarios (
  id SERIAL PRIMARY KEY, -- SERIAL para AUTO_INCREMENT
  email VARCHAR(60),
  password CHAR(60) -- Se puede dejar 'CHAR(60)' para contraseñas hasheadas
);
...
('Juan', 'De la torre', '091390109'),
('KAREN ACT', 'Perez', '0123456789');
